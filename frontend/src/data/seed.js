...
export const dashboardMetrics = {
    ...
    failedTasks24h: tasks.filter(t => t.status === 'FAILED' &&
        t.startedAt &&
        new Date(t.startedAt) > new Date(Date.now() - 24 * 60 * 60 * 1000)).length,
    ...
    exceptions: {
        ...
        waitingLong: tasks.filter(t => t.status === 'WAITING_FOR_DEPENDENCY' &&
            t.startedAt &&
            new Date(t.startedAt) < new Date(Date.now() - 15 * 60 * 1000)),
        staleAgents: agents.filter(a => (a.status === 'FAILED' || new Date(a.lastHeartbeat) < new Date(Date.now() - 5 * 60 * 1000)))
    }
};
